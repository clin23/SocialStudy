...
const Sidebar = ({
  ...
  return (
    <div
      className={`fixed top-0 left-0 h-screen bg-gray-800 text-indigo-100 flex flex-col transition-all duration-300 ease-in-out z-40 shadow-lg ${
        isOpen ? "w-48" : "w-20"
      }`}
    >
      ...
      {currentUser && (
        ...
        >
          ...
          {isOpen && (
            <p className="text-sm font-medium text-center truncate mb-1">
              {currentUser.displayName}
            </p>
          )}
          <button
            onClick={handleLogout}
            className={`w-full flex items-center space-x-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors text-red-400 hover:bg-red-700 hover:text-red-200 ${
              !isOpen ? "justify-center" : ""
            }`}
            title="Logout"
          >

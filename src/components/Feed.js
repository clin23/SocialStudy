...
const Feed = ({
  ...
  useEffect(() => {
    ...
    if (!currentUser.following || currentUser.following.length === 0) {
      q = query(collection(db, getStudySessionsPath()));
    } else {
      const followingList =
        currentUser.following.length > 30
          ? currentUser.following.slice(0, 30)
          : currentUser.following;
      if (currentUser.following.length > 30) {
        console.warn(
          "Feed query: 'following' array has more than 30 UIDs. Firestore 'in' query is limited to 30. Truncating list for query."
        );
      }
      q = query(
  ...
  if (isLoading && feedSessions.length === 0)
    return (
            ...
            <div className="text-center py-10">
              ...
              {(!currentUser ||
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Find users to follow in the &apos;Users&apos; tab to

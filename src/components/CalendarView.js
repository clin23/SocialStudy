          ...
          >
            <span
              className={`text-xs font-semibold ${
                isToday
                  ? "bg-indigo-500 text-white rounded-full h-5 w-5 flex items-center justify-center"
                  : ""
              }`}
            >
              {day.getDate()}
            ...
            <div className="mt-1 flex-grow overflow-y-auto text-left pr-1">
              {tasksForDay.map((task) => (
                <div
                  key={task.id}
                  className={`text-xs px-1 py-0.5 mb-1 rounded-md truncate ${
                    task.completed
                      ? "bg-green-800 text-green-300 line-through"

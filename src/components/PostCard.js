...
const PostCard = ({ session, currentUser, onOpenPostModal }) => {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-indigo-400 mb-0.5">
        {session.title}
      </h3>
      {session.sessionType && (
      ...
      <div className="flex justify-between items-center text-xs text-gray-300 mb-1">
        <span>
          <Clock className="inline h-3 w-3 mr-1 text-gray-400" /> Duration:{" "}
          {formatDuration(session.duration)}
        </span>
        {typeof session.focusScore === "number" && (
          <span className="flex items-center">
            <Star className="inline h-3 w-3 mr-1 text-yellow-400" /> Focus:{" "}

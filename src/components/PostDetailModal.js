...
import { getLikesPath, getCommentsPath } from "../firebase/paths";
    ...
    const unsubscribeLikes = onSnapshot(likesQuery, (snapshot) => {
      ...
      if (currentUser && currentUser.uid) {
        setIsLikedByCurrentUser(
          likesData.some((like) => like.userId === currentUser.uid)
        );
      }
    });
      ...
      <div className="space-y-3">
        <div className="flex items-start mb-2">
          <img
            src={
              ...
              `https://placehold.co/40x40/374151/E5E7EB?text=${
                session.userDisplayName?.charAt(0).toUpperCase() || "U"
              }`
            }
            ...
            onError={(e) => {
              ...
              e.target.src = `https://placehold.co/40x40/374151/E5E7EB?text=${
                session.userDisplayName?.charAt(0).toUpperCase() || "U"
              }`;
            }}
          />
          <div className="flex-grow">
            <p className="font-semibold text-gray-100">
              {session.userDisplayName || "Anonymous User"}
            </p>
        ...
        <div className="flex flex-wrap justify-between items-center text-sm text-gray-300 mb-2 border-b border-gray-700 pb-2">
          <span className="flex items-center mb-1 sm:mb-0 mr-3">
            <Clock className="inline h-4 w-4 mr-1 text-gray-400" /> Duration:{" "}
            {formatDuration(session.duration)}
          </span>
          {typeof session.focusScore === "number" && (
            <span className="flex items-center mb-1 sm:mb-0">
              <Star className="inline h-4 w-4 mr-1 text-yellow-400" /> Focus:{" "}
          ...
          <div className="my-3 p-3 bg-gray-700 rounded-lg">
            ...
            {session.mediaType?.startsWith("image/") && (
              <div className="flex flex-col items-center">
                ...
                <p className="text-xs text-gray-400">
                  (Image: {session.mediaName})
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  Full image display requires Firebase Storage integration and a
                  valid URL.
                </p>
              </div>
            )}
            {session.mediaType?.startsWith("video/") && (
              <div className="flex flex-col items-center">
                ...
                <p className="text-xs text-gray-400">
                  (Video: {session.mediaName})
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  Full video playback requires Firebase Storage integration and
                  a valid URL.
                </p>
              </div>
            )}
            {!session.mediaType?.startsWith("image/") &&
              !session.mediaType?.startsWith("video/") && (
                <p className="text-gray-300 text-sm">
                  {session.mediaName} (Unsupported preview)
                </p>
              )}
          </div>
        )}
        ...
        {session.sessionPlan && (
          <div className="mb-2">
            <p className="text-md font-medium text-indigo-300 mb-1">
              Session Plan:
            </p>
            <p className="text-gray-200 text-sm whitespace-pre-wrap bg-gray-700 p-2 rounded-md">
              {session.sessionPlan}
            </p>
          </div>
        )}
          ...
          <div className="mb-2">
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {session.distractions.map((dist, index) => (
                <span
                  key={index}
                  className="text-sm bg-gray-700 text-gray-300 px-2 py-0.5 rounded-full"
                >
                  {dist}
                </span>
              ))}
          ...
          >
            <ThumbsUp
              className={`h-5 w-5 mr-1 ${
                isLikedByCurrentUser ? "fill-current text-red-400" : ""
              }`}
            />
                  ...
                  <div className="flex items-start">
                    <img
                      src={
                        ...
                        `https://placehold.co/32x32/374151/E5E7EB?text=${
                      ...
                      onError={(e) => {
                        e.target.onerror = null;
                        e.target.src = `https://placehold.co/32x32/374151/E5E7EB?text=${
                          comment.userDisplayName?.charAt(0).toUpperCase() ||
                          "U"
                        }`;
                      }}

...
import { getUserTodosPath } from "../firebase/paths";
...
const TodoList = ({
  ...
  const todosCollectionRef = useMemo(() => {
    if (currentUser && currentUser.uid && db) {
      return collection(db, getUserTodosPath(currentUser.uid));
    }
    return null;
  }, [currentUser]);
  ...
  const handleToggleComplete = async (todoId, currentStatus) => {
    ...
    const todoDocRef = doc(db, getUserTodosPath(currentUser.uid), todoId);
    try {
  ...
  const handleOpenEditModal = (todo) => {
    ...
    setCurrentDueDate(
      todo.dueDate ? todo.dueDate.toDate().toISOString().split("T")[0] : ""
    );
  ...
  const handleSaveTaskDetails = async () => {
    ...
    if (
      currentLinksInput.trim() &&
      !newLinks.includes(currentLinksInput.trim())
    ) {
    ...
    try {
      await updateDoc(todoDocRef, {
        ...
        dueDate: currentDueDate
          ? Timestamp.fromDate(new Date(currentDueDate + "T00:00:00"))
          : null,
  ...
  const handleAddLinkToModal = () => {
    if (
      currentLinksInput.trim() &&
      !currentLinks.includes(currentLinksInput.trim())
    ) {
  ...
  return (
    <div
      className={`p-4 rounded-lg shadow-md h-full flex flex-col ${
        isFullPage ? "bg-gray-800" : "bg-gray-800"
      }`}
    >
      <h3
        className={`text-xl font-semibold text-indigo-400 mb-3 ${
          isFullPage ? "text-center text-2xl" : ""
        }`}
      >
          ...
          {todos.map((todo) => (
            <li
              ...
              className={`p-2.5 rounded-md transition-colors duration-150 ${
                todo.completed ? "bg-gray-700" : "bg-gray-600 hover:bg-gray-500"
              } cursor-pointer`}
              onClick={(e) => {
                if (
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  <button
                    ...
                    title={
                      todo.description ? "Edit/View details" : "Add details"
                    }
                  >
                    <Edit3 size={16} />
                  </button>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex">
                ...
                <button
                  type="button"
                  onClick={handleAddLinkToModal}
                  className="px-3 py-2 bg-indigo-500 text-white rounded-r-md hover:bg-indigo-600 text-sm"
                >
                  <PlusCircle size={18} />
                </button>
              </div>
              {currentLinks.length > 0 && (
                <div className="mt-2 space-y-1 max-h-20 overflow-y-auto">
                  ...
                  {currentLinks.map((link, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between text-xs text-indigo-300 bg-gray-700 px-2 py-1 rounded"
                    >
                      <a
                        href={link}
                        target="_blank"
                      ...
                      <button
                        onClick={() => handleRemoveLinkFromModal(link)}
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-gray-700">
              ...
              >
                <PlayCircle size={16} className="mr-2" /> Start Session for this
                Task
              </button>
            </div>
          </div>
